"""DQN agent and shared Q-network."""
...
class DQNAgent(BaseAgent):
    """Deep Q-Network with a target network and experience replay."""
    ...
    def __init__(self, state_size, action_size, max_order=20, buffer_size=10000,
        ...
        """Initialise Q-net, target-net, optimiser, and replay buffer.

        ``tau`` is the soft-update coefficient for the target network.
        ``update_every`` controls both the learning frequency and the target
    ...
    def act(self, state, epsilon=0.0):
        """Epsilon-greedy action selection over discrete order quantities."""
    ...
    def step(self, state, action, reward, next_state, done):
        """Store a transition and trigger learning every ``update_every`` steps."""
    ...
    def learn(self, experiences):
        """One Q-learning update from a sampled batch.
    ...
    def update(self, *args, **kwargs):
        """No-op: DQN learns online inside ``step``."""
        pass

    def save(self, path):
        """Persist Q-net, target-net, and optimiser state."""
    ...
    def load(self, path):
        """Load Q-net, target-net, and optimiser state. Returns False if file is missing."""
